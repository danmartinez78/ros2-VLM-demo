...
TEST(IpcProtocol, RequestHeaderV3FieldsDefaultToInlineMode)
{
  ipc::RequestHeader header;

  // Schema v3 request header defaults must match inline (backward-compatible) delivery.
  ...
  EXPECT_EQ(header.sequence_type, 0U);
  EXPECT_EQ(header.timestamp_count, 0U);
  EXPECT_DOUBLE_EQ(header.fps, 0.0);
