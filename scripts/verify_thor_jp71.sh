...
if [[ " ${passthrough_args[*]} " == *" --isaac-ros "* ]]; then
  check "Isaac ROS RT-DETR package available" bash -c '
    ...
    set -u
    ros2 pkg prefix isaac_ros_rtdetr >/dev/null 2>&1 || ros2 pkg prefix isaac_ros_examples >/dev/null 2>&1
  '
fi
...
if [[ "${run_standalone_smoke}" -eq 1 ]]; then
  ...
  echo "WARN  Standalone Edge-LLM smoke test skipped (pass --smoke-image /absolute/path.jpg to execute it)"
fi
